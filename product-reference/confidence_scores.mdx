...
For arrays, a confidence score is calculated for each individual value in the array. To view these confidences, hover over the cell the value is contained in. The overall confidence score for the array is the minimum of all the individual array values. This means that a low confidence score for an array does not necessarily mean that all of the values in the array are low confidence. It could mean that one or more of the values in the array are low confidence, but the rest are high confidence. For example, if you have an array of line items on an invoice, the confidence score for the array could be low because the model is not confident in the line item quantity, but it is confident in the line item description and price.
